...
- **`/art-icon`**, **`/art-prop`**, **`/art-character`**, **`/art-terrain`**, **`/art-tileset`**, **`/art-ui`**, **`/art-background`** -- Generate game art locally via the `gameart` MCP server (ComfyUI + Ollama on this machine; no cloud service, no credits). Shared pick-and-refine loop in `research/art/local_workflow.md`, living prompt presets in `research/art/local_style_presets.md`. `/art-tileset` builds dual-grid 16-tile sets matching `level/src/terrain.rs::wang_index`.
- **`/art-animation`** -- Make an approved sprite move: character idle/walk/run cycles and looping scenery motion (swaying flora, water, torches). Owns the per-kind sheet grids and `scripts/build_sheet.py`, which assembles frames into the exact grid the engine declares (`just sheet` / `just sheet-check`). Frame animation is the default for scenery whose shape changes; the transform/alpha systems in `level/src/grass.rs` and `level/src/water/animation.rs` are an accessory for tiled surfaces and wind-reactive motion. Playback components live in `models/src/sprite_anim.rs`.
